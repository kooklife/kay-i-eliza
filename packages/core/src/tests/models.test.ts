...
describe("Model Retrieval Functions", () => {
    describe("getModel function", () => {
        test("should retrieve correct models for different providers and classes", () => {
            ...
            expect(getModel(ModelProviderName.ANTHROPIC, ModelClass.LARGE))
                .toBe(
                    "claude-3-sonnet-20240229",
                );
            expect(getModel(ModelProviderName.LLAMACLOUD, ModelClass.MEDIUM))
                .toBe(
                    "meta-llama-3.1-8b-instruct",
                );
        });
        ...
        test("should handle environment variable overrides", () => {
            expect(getModel(ModelProviderName.OPENROUTER, ModelClass.SMALL))
                .toBe("mock-small-model");
            expect(getModel(ModelProviderName.OPENROUTER, ModelClass.LARGE))
                .toBe("mock-large-model");
            expect(getModel(ModelProviderName.ETERNALAI, ModelClass.SMALL))
                .toBe("mock-eternal-model");
        });

        test("should throw error for invalid model provider", () => {
            expect(() => getModel("INVALID_PROVIDER" as any, ModelClass.SMALL))
                .toThrow();
        });
    });
